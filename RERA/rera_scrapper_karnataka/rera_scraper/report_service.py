"""Orchestrate Karnataka RERA report + scrape into one MongoDB document."""
...
async def scrape_entity_for_report(
    ...
    *,
    ...
    # Search in local KA_allprojects collection first
    emit("Searching local KA_allprojects index...")
    ...
    try:
        ...
        emit(f"Failed to query local index: {e}", "warning")
    finally:
        search_service.close()

    # If not found locally, we would run a live search/scrape
    emit("No local results; attempting live scraper is skipped in report mode.")
    return []
