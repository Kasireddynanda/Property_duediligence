"""CLI entry point for Telangana RERA scraper."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape Telangana RERA project data (session-persistent Playwright scraper)."
    )
    parser.add_argument(
        "projects",
        nargs="*",
        help="Project name(s) to search via #Project (min 4 characters each).",
    )
    parser.add_argument(
        "--promoter",
        nargs="+",
        metavar="NAME",
        help="Search by promoter name via #promoter_name.",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        "--no-follow-promoter",
        action="store_true",
        help="Do not re-search using Promoter Organization Name from scraped details.",
    )
    parser.add_argument(
        "--mongo-uri",
